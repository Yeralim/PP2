...
RETURNS TABLE(id int, user_name VARCHAR, surname VARCHAR, phone VARCHAR) AS
$$
BEGIN
    RETURN QUERY
    SELECT phonebook.id, phonebook.user_name, phonebook.surname, phonebook.phone
    FROM phonebook
...
CREATE OR REPLACE PROCEDURE insert_data(name VARCHAR, new_phone VARCHAR) AS
$$
BEGIN
    UPDATE phonebook SET phone = new_phone WHERE user_name = name;
    IF NOT FOUND THEN
        INSERT INTO phonebook (user_name, surname,  phone)
...
def insert(name, new_phone):
    cur.execute("CALL insert_data(%s, %s)", (name, new_phone,))

...
LMBpressed = False  #нажатия левой кнопки мыши
RMBpressed = False  #нажатия правой кнопки мыши
...
curr_color = colorRED  #color kisti
clock = pygame.time.Clock()  #таймер

done = False  # main game process
while not done:
  for event in pygame.event.get():
    if event.type == pygame.QUIT:
      done = True
    elif event.type == pygame.KEYDOWN:  #нажатия клавиш
      ...
      elif event.key == pygame.K_c:  #Очищаем экран
        screen.fill(colorBLACK)
        rects.clear()
        circles.clear()
        squares.clear()
    ...
    elif event.type == pygame.MOUSEBUTTONUP:  #отпускать кнопкум ышки
      if event.button == 1:
        LMBpressed = False
        if mode == "rect":
          ...
          right_triangles.append((right_tri.copy(), curr_color))
        elif mode == "equilateral_triangle":
          equilateral_triangles.append((equilateral_tri.copy(), curr_color))
        elif mode == "rhombus":
  ...
  for rhomb, color in rhombuses:
    pygame.draw.polygon(screen, color, rhomb, 2)

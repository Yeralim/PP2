...
def draw_grid_chess():  #рисуем шахматную доску
    colors = [colorWHITE, colorGRAY]

    for i in range(HEIGHT // 2):
        for j in range(WIDTH // 2):
            pygame.draw.rect(screen, colors[(i + j) % 2], (i * CELL, j * CELL, CELL, CELL)) #Чередование цветов colors[(i + j) % 2] даёт шахматный эффект
...
class Snake:
  def __init__(self):
    self.body = [Point(10, 11), Point(10, 12), Point(10, 13)] #штшешфд положение
    self.dx, self.dy = 1, 0  # Направление движения
    self.grow = False  # Флаг для роста змейки если тру то оно растет
  ...
  def check_collision(self, food):
    if self.body[0].x == food.pos.x and self.body[0].y == food.pos.y: #Если голова змейки matcheswith coordinates еды змейка увеличивается.
      self.grow = True  # Увеличиваем змейку
      return True
    return False

  def check_self_collision(self):   #cheks,пересекла ли голова любую часть своего тел
    head = self.body[0]
    return any(segment.x == head.x and segment.y == head.y for segment in self.body[1:])

  def check_wall_collision(self): #if head isnout of granitza ,возвращает тру
    head = self.body[0]
...
while running:
  ...
  for event in pygame.event.get():
    if event.type == pygame.QUIT:
      running = False
    elif event.type == pygame.KEYDOWN:  # меняет направление при нажатии
